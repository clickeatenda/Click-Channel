...
# -*- coding: utf-8 -*-
"""
Script para ajustar TODAS as labels das 60 issues com o novo padrão
"""
...
issues_labels = {
    ...
    "EPG mostrado somente em CANAIS": ["Aplicacao Mobile", "feature", "media"],
    "Notificacao de programa favorito": ["Aplicacao Mobile", "feature", "media"],
    
    # Performance - melhoria - media
    ...
    "Agrupamento de variantes por canal": ["Aplicacao Mobile", "feature", "media"],
    
    # Android TV - feature - baixa
...
for issue in repo.get_issues(state='all'):
    if issue.title in issues_labels:
        labels = issues_labels[issue.title]
        
        try:
            # Remover labels antigas
            for old_label in issue.labels:
                issue.remove_from_labels(old_label)
            
            # Adicionar labels novas
            for label in labels:
                issue.add_to_labels(label)
            
            print(f"OK #{issue.number} - {issue.title[:40]} -> {labels}")
            updated += 1
            
        except Exception as e:
            print(f"ERRO #{issue.number}: {str(e)[:50]}")

print(f"\n{updated} issues atualizadas com labels corretas!")
